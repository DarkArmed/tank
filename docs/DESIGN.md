...
- 模拟层按 PRD 推进终局展示时序；`gameOver` 和 `completed` 事件分别表示对应终局展示已经完成，而不是刚进入同名场景。
- 每局只产生一次对应的终局完成事件；终局展示完成后继续调用 `tick` 不得重复产生。
...
- 应用壳收到 `gameOver` 或 `completed` 事件后结束当前模拟实例，并按 PRD 返回 `tankSelect`；不得根据同名快照自行复制终局计时。
